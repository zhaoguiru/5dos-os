...
// v0.4.3 "OS with Hooks & K-Priority" (Lock-Safe + Kthread-Protect)
...
static void update(struct timer_list *tm)
{
    ...
    struct entity *ent;
    int i = 0;
    u64 now_ns = ktime_get_ns(), nB, nR, nS, nD, nI, nI_th, temp;
    unsigned int s;
    ...
    for (int j = 0; j < g.n; j++) {
        ...
        nI_th = (g.max_I > I_ONTO_FLOOR) ? div64_u64(ent->I * SCALE, g.max_I) : 0;

        temp = nB * nR; do_div(temp, SCALE);
        temp = temp * nS; do_div(temp, SCALE);
        temp = temp * nD; do_div(temp, SCALE);
        temp = temp * nI_th; do_div(temp, SCALE);
        ent->kappa_theory = temp;
        ...
        else if (ent->kappa < SCALE/1000) ent->state = ST_LATENT;
        else ent->state = ST_BIRTH;

        g.cnt[ent->state]++;
        sum += ent->kappa;
    }
    g.avg_kappa = g.n ? div64_u64(sum, (u64)g.n) : 0;
...
static int show_top(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "Top Active Entities by Kappa (Ontological Floor Applied)\n");
    seq_printf(m, "=========================================================================\n");
    seq_printf(m, "RANK PID     COMM                      KAPPA   K_RAW   STATE   I(%%)  TH\n");
    seq_printf(m, "-------------------------------------------------------------------------\n");
    for (int i = 0; i < g.n && c < 20; i++) {
        ...
        seq_printf(m, "%-4d %-7d %-20s %2llu.%06llu %2llu.%06llu %-7s %3llu.%02llu %3d\n",
            ...
            sort_ptr[i]->kappa_theory / SCALE, sort_ptr[i]->kappa_theory % SCALE,
...
{
    read_lock(&g_rwlock);
    seq_printf(m, "5DOS-OS Kernel Module v0.4.3 (Hooks + K-Priority)\n");
    seq_printf(m, "============================================================\n");
    ...
    for (int i = 0; i < ST_MAX; i++)
        seq_printf(m, "  %-7s : %4d\n", st_name[i], g.cnt[i]);
    seq_printf(m, "\nTheory:\n");
    seq_printf(m, "  kappa_effective = nB * nR * nS * nD * nI\n");
    seq_printf(m, "  nI = max(I_raw, 0.01%%) / (max_I + 0.01%%)\n");
    seq_printf(m, "  kappa_theory = pure product (for academic comparison)\n");
...
static int __init init_5dos_os(void)
{
    ...
    pr_info("5DOS-OS: loaded v0.4.3 (hooks + K-priority + lock-safe)\n");
    return 0;
}
...
MODULE_DESCRIPTION("5DOS Operating System with Hooks and K-Priority");
