...
// v2.0 "Built-in Edition" (Lock-Safe + Kthread-Protect + Inner Synergy)
...
#include <linux/module.h>   /* for initcall 宏 */
#include <asm/div64.h>
...
{
    read_lock(&g_rwlock);
    seq_printf(m, "5DOS-OS Kernel Module v2.0 (Built-in, Ratio Method)\n");
    seq_printf(m, "============================================================\n");
...
static int __init init_5dos_os(void)
{
    ...
    pr_info("5DOS-OS: loaded v2.0 built-in (ratio method + inner synergy)\n");
    return 0;
}
...
/* 编译进内核时，使用 subsys_initcall 在子系统初始化阶段加载 */
subsys_initcall(init_5dos_os);
